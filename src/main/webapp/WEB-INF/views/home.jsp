		...
		<form class="form-horizontal" name='homeForm' method="post"	action="/member/login" >
			<c:if test="${member == null}">
				...
				<div class="col-sm-3">
						...
						<button id="registerBtn" type="button" class="btn btn-secondary">Sign
							up</button>
				</div>
				</div>
			</c:if>
			<c:if test="${member != null }">
				<div class="form-group" style="margin-right: 110px;">
					...
					<p style="color:white;float: right;">${member.userId}님 환영합니다.</p><br><br><br>
			...
			<c:if test="${msg == false}">
				<p style="color: red; margin-left: 80px; margin-top: 35px;">로그인
					실패! 아이디와 비밀번호 확인해주세요.</p>
			</c:if>
		</form>
		<!-- Extra -->
		<div id="extra">
				...
				<div class="row">
					<div class="col">
						...
						<div class="box">
							<table class="table table-hover" id="tb"
								style="text-align:center; width:480px;font-size:14px; margin-top:-48px; margin-bottom:-48px; margin-left:-10px; border:1px solid white">
								...
								<%
										...
										for (Element e : lis1) {
											for (int i = 0; i < 2; i++) {
												if (i == 1) {
													out.print("<td><a href='https://sports.news.naver.com/wfootball/index.nhn" + e.attr("href")
															+ "' target='_blank'>'" + e.select("a").text() + "'</a></td>");
												} else {
					...
					<div class="col">
						...
						<div id="main" style="margin-left: 30px">
							...
							<table class="table table-hover" id="tb"
								style="text-align: center; border: 1px solid white; background-color: white; font-size: 13px;">
								<tr>
									<%
										for (Element e : lis1) {
								...
								<%
									for (Element e : lis2) {
										out.print("<tr style='background:#6f8dd9;'>");
										int cnt = 0;
										for (int i = 0; i < 220; i++) {
											if (cnt % 11 == 1) {
												out.print("");
												...
												if (cnt % 11 == 0 && cnt != 0) {
													if (cnt >= 11 * 17) {
														out.print("</tr><tr style='background:#c0c0c0;'>");
													} else if (cnt <= 11 * 3) {
														out.print("</tr><tr style='background:#6f8dd9;'>");
													} else if (cnt == 11 * 4) {
														out.print("</tr><tr style='background:#80d882'>");
													} else {
														out.print("</tr><tr>");
													}
												}
